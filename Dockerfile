...
ARG WYOMING_PIPER_VERSION="1.5.3"
...
RUN \
    ...
    \
    SITE_PACKAGES=$(/app/bin/python3 -c "import site; print(site.getsitepackages()[0])")
...
RUN \
    ...
    cp /tmp/wyoming_piper_handler.diff handler.py && \
    rm /tmp/wyoming_piper_handler.diff && \
    for file in /tmp/wyoming_piper*.diff; do patch -p0 --forward < $file; done && \
    rm -rf /tmp/* /root/.cache /var/lib/apt/lists/* /app/include/*.hpp /app/include/cpu_provider_factory.h /app/include/provider_options.h
